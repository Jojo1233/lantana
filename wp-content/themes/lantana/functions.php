...
class Kama_Breadcrumbs {
    ...
    function get_crumbs( $sep, $l10n, $args ){
        ...
        // Таксы, вложения и не древовидные записи
            ...
            if( is_singular() ){
                ...
                if( is_attachment() && $post->post_parent ){
                    ...
                    if( is_post_type_hierarchical( $post->post_type ) ){
                        $attach_post_crumbs = $this->_page_crumbs( $post );
                    }
                }
            ...
            if( isset($attach_post_crumbs) ){
                $out = $attach_post_crumbs . sprintf( $linkpatt, get_permalink($post->post_parent), get_the_title($post->post_parent) );
                $out = $this->_add_title( $out, $post );
            }
            // термин
            elseif( $term && isset($term->term_id) ){
                ...
                if( is_attachment() ){
                    if( ! $post->post_parent )
                        ...
                        $_crumbs = $this->_tax_crumbs( $term, 'from_self' ); // с sep на конце
                        $tit = sprintf( $linkpatt, get_permalink($post->post_parent), get_the_title($post->post_parent) );
                        $out = $_crumbs . $this->_add_title( $tit, $post );
                    }
                }
                // single
                elseif( is_single() ){
                    $_crumbs = $this->_tax_crumbs( $term, 'from_parent' ); // с sep на конце
                    $out = $_crumbs . sprintf( $linkpatt, get_term_link($term), $term->name );
                    $out = $this->_add_title( $out, $post );
                }
                ...
                elseif( ! is_taxonomy_hierarchical( $term->taxonomy ) ){
                    // метка
                    if( is_tag() )
                        $out = $this->_add_title('', $term, sprintf( $loc->tag, $term->name ) );
                    // такса
                    elseif( is_tax() ){
                        $post_label = $ptype->labels->name;
                        $tax_label = $GLOBALS['wp_taxonomies'][ $term->taxonomy ]->labels->name;
                        $out = $this->_add_title('', $term, sprintf( $loc->tax_tag, $post_label, $tax_label, $term->name ) );
                    }
                }
                // древовидная такса: рубрика
                else {
                    $_crumbs = $this->_tax_crumbs( $term, 'from_parent' ); // с sep на конце
                    $out = $this->_add_title( $_crumbs, $term, $term->name );
                }
            }
            // запись, и у нее нет термина
            elseif( is_singular() )
    ...
    function _page_crumbs( $post ){
        ...
        return $this->_add_title( implode($this->arg->sep, $crumbs), $post );
    }

    function _tax_crumbs( $term, $start = 'from_self' ){
        $termlinks = array();
        $term_id = ($start === 'from_parent') ? $term->parent : $term->term_id;
        while( $term_id ){
    ...
    function _add_title( $add_to, $obj, $term_title = '' ){
        $arg = & $this->arg; // жить надо проще...
        $title = $term_title ? esc_html($term_title) : esc_html($obj->post_title);
